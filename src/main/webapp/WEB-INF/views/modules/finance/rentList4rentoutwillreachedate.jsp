		...
		function exportExcel4base(){
			top.$.jBox.confirm("确认要导出房屋包租数据吗？","系统提示",function(v,h,f){
				if(v=="ok"){
					...
					$("#searchForm").attr("action","${ctx}/finance/rent/export4rentoutwillreachedate");
					$("#searchForm").submit();
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/finance/rent/">包租明细【租出房租即将到期】列表</a></li>
	</ul>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="rent">
			<tr>
				...
				<td>${rent.house.tenant.name}</td>
				<td>${rent.house.tenant.telephone}</td>
