...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			$("#searchForm").attr("action","${ctx}/finance/stats/vacantPeriod4PersonByMonth");
			$("#searchForm").submit();
        	return false;
        }
...
<body>
	<form:form id="searchForm" modelAttribute="vacantPeriod" action="${ctx}/finance/stats/vacantPeriod4PersonByMonth" method="post" class="breadcrumb form-search">
	...
	<div id="pro_search" style="margin-top:10px;${'true' eq paramMap.showHighSearch?'':'display:none;'}">
		...
		<input id="rentout_sdate_begin" name="rentout_sdate_begin" type="text"  maxlength="20" class="input-small Wdate"
				value="${paramMap.rentout_sdate_begin}" onclick="WdatePicker({dateFmt:'yyyy-MM-01',isShowClear:false});"/>
	</div>
	</form:form>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${list}" var="var">
			<tr>
				<td><a href="#" onclick="openWindow('${ctx}/finance/stats/vacantPeriodDetail4PersonByMonth?personid=${var.person.id}&rentout_sdate_begin=${paramMap.rentout_sdate_begin}','${var.person.name}')">${var.person.name}</a></td>
