...
<script type="text/javascript">
		$(document).ready(function() {
			$("input[name=id]").each(function(){
				var landlordSelect = null;
				if (top.mainFrame.financeMainFrame){
					landlordSelect = top.mainFrame.financeMainFrame.landlordSelect;
				}else{
					landlordSelect = top.mainFrame.landlordSelect;
				}
				for (var i=0; i<landlordSelect.length; i++){
					if (landlordSelect[i][0]==$(this).val()){
						this.checked = true;
					}
				}
				$(this).click(function(){
					...
					if (top.mainFrame.financeMainFrame){
						top.mainFrame.financeMainFrame.landlordSelectAddOrDel(id, title, telephone, card);
					}else{
						top.mainFrame.landlordSelectAddOrDel(id, title, telephone, card);
					}
				});
			});
		});
